...
set -ex
torchrun --nproc_per_node=2 --master_port=$MASTER_PORT exps/baseline_mdlm.py --config=_default.yaml method=baseline n_runs=$N_RUNS n_groups=16 group_size=1 subsample_k=4 "$@" 2>&1 | tee "$BASELINE_LOG"
set +ex
...
for w_int in "${INTERACTION_VALUES[@]}"; do
    ...
    set -ex
    torchrun --nproc_per_node=2 --master_port=$MASTER_PORT single_run_mdlm.py --config=_default.yaml method=greedy_map _w_interaction=$w_int n_runs=$N_RUNS n_groups=4 group_size=4 "$@" 2>&1 | tee "$INTERACTION_LOG"
    set +ex
