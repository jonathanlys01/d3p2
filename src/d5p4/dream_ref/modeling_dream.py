...
class DreamRotaryEmbedding(nn.Module):
    ...
    @staticmethod
    def _compute_default_rope_parameters(config, device, **_kwargs):
        """Transformers 5 removed the default initializer from ROPE_INIT_FUNCTIONS."""
...
class DreamModel(DreamGenerationMixin, DreamPreTrainedModel):
    # Transformers 5 expects a mapping here. Dream does not tie the input and
    # output embeddings in the published checkpoint.
    _tied_weights_keys = {}
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        outputs = self.model(
            ...
            use_cache=use_cache,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states and not last_hidden_state_only,
            return_dict=return_dict,
            cache_position=cache_position,
        )

        hidden_states = outputs[0]
        returned_hidden_states = (hidden_states,) if last_hidden_state_only else outputs.hidden_states
