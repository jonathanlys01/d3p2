...
SUPPORTS_FLASH = flash_attn is not None and torch.cuda.is_available() and torch.cuda.get_device_properties(0).major >= 8
...
def apply_rotary_pos_emb(qkv, cos, sin):
    ...
    return qkv * cos.to(qkv.dtype) + rotate_half(qkv) * sin.to(qkv.dtype)
...
def regular_attention_multi_headed(qkv):
    ...
    return attention_output.contiguous().view(batch_size, seq_len, num_heads * head_dim)


class DDiTBlock(nn.Module):
    def __init__(self, dim, n_heads, cond_dim, mlp_ratio=4, dropout=0.1, use_flash_attn=True):
        super().__init__()
    ...
    def forward(self, x, rotary_cos_sin, c, seqlens=None):
        ...
        qkv = self.attn_qkv(x)
        qkv = qkv.view(batch_size, seq_len, 3, self.n_heads, qkv.shape[-1] // (3 * self.n_heads))
        with torch.amp.autocast(device_type="cuda", enabled=False):
            cos, sin = rotary_cos_sin
            qkv = apply_rotary_pos_emb(qkv, cos.to(qkv.dtype), sin.to(qkv.dtype))
        if SUPPORTS_FLASH and self.use_flash_attn:
            qkv_flat = qkv.reshape(batch_size * seq_len, 3, self.n_heads, qkv.shape[-1])
            if seqlens is None:
            ...
            x = flash_attn.flash_attn_varlen_qkvpacked_func(qkv_flat, cu_seqlens, seq_len, 0.0, causal=False)
            x = x.reshape(batch_size, seq_len, self.n_heads * qkv.shape[-1])
        else:
            ...
            x = x.transpose(1, 2).contiguous().view(batch_size, seq_len, self.n_heads * qkv.shape[-1])
...
class DITBackbone(nn.Module):
    ...
    def forward(self, indices, sigma, cond=None, x_emb=None, output_hidden_states=False):
        ...
        **kwargs,
        ...
        if return_dict:
            return modeling_outputs.MaskedLMOutput(
                logits=logits, hidden_states=all_hidden_states if output_hidden_states else None, loss=None
            )
