...
def restore_model(model):
    ...
    if model.ema:
        model.ema.restore(itertools.chain(model.backbone.parameters(), model.noise.parameters()))
...
def compute_generative_perplexity(model, text_sample: str, eval_model_name: str = "gpt2-large") -> float:
    ...
    eval_model = AutoModelForCausalLM.from_pretrained(**model_args).eval().to(model.device)
