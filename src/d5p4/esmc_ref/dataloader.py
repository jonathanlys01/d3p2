...
def get_tokenizer(config):
    if config.data.tokenizer_name_or_path == "text8":
        ...
        tokenizer = transformers.BertTokenizer.from_pretrained("bert-base-uncased", cache_dir=config.data.cache_dir)
    else:
        tokenizer_path = config.data.tokenizer_name_or_path
        tokenizer_args = {"cache_dir": config.data.cache_dir}
        if os.path.isdir(tokenizer_path):
            tokenizer_args["local_files_only"] = True
        tokenizer = transformers.AutoTokenizer.from_pretrained(tokenizer_path, **tokenizer_args)
