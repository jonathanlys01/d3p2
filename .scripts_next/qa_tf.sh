...
python \
  ...
  group_size=1 \
  comment="TruthfulQA Indep ${INDEP_N_GROUPS}x1" \
...
python \
  ...
  comment="TruthfulQA D5P4 ${N_GROUPS}x${GROUP_SIZE} w=${D5P4_W_INTERACTION}" \
...
python \
  ...
  comment="TruthfulQA D5P4-P ${N_GROUPS}x${GROUP_SIZE} w=${D5P4_W_INTERACTION} guidance_end=${PARTIAL_GUIDANCE_END}" \
